      ...
      <div style={styles.mainContent}>
        ...
        <div style={styles.codeCard}>
          <div style={styles.codeCardHeader}>
            <span style={styles.fileName}>{`Snippet (${snippet?.language})`}</span>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  if (typeof snippetId !== 'string') {
    return { props: { snippet: null, error: "Invalid snippet ID." } };
  }

  try {
    const data: string | null = await kv.get(snippetId);

    if (!data) {
      return { props: { snippet: null, error: "Snippet not found or has expired." } };
    }
